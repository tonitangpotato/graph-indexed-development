...
> **Numbering note (2026-04-26):** Branch `iss-001-002-revive` accidentally reused ISS-001 / ISS-002 for new work. The collision was resolved by renumbering the new work to **ISS-048** (CodeNode `test_default` helper, originally filed as "new ISS-001") and **ISS-049** (configurable clustering edge weights, originally filed as "new ISS-002"). The original ISS-001 (`gid extract --output` flag) and ISS-002 (LSP client / receiver-type matching) keep their numbers — those issues never had standalone files; the entries below are the only record. All 36 issues are closed.
>
> Source code, cross-references, and issue files all use post-renumbering IDs (ISS-048 / ISS-049).
...
## ISS-038 [bug] [P0] [closed]
**Ritual `implement` phase reports success despite zero file changes** *(2026-04-25)*

**发现日期**: 2026-04-25
**关闭日期**: 2026-04-25
**发现者**: RustClaw (ritual `r-af714a` produced 0 file diffs after burning ~23k tokens, finished `Done` with `failed_phase = None`)
**组件**: `gid-core/src/ritual/{api_llm_client.rs, v2_executor.rs}`
**Cross-repo**: rustclaw ISS-025

### 问题
`implement` phase 在 LLM 没做任何 `Write`/`Edit` tool call 的情况下仍返回 `SkillCompleted { artifacts: [] }`。下游 `verify` 对未变更的树跑过，state machine 抵达 `Done` 无 `failed_phase` —— **false success**。比真失败更糟：失败会让 operator 调查，false success 让人以为完成了。

### Root cause
`api_llm_client.rs` 把 artifact 跟踪 punt 给 executor 层，但 `v2_executor` 从来没接手。契约半实现，filesystem 永远不被验证。

### 修复（已合并，commit `52a84ee`）
Filesystem snapshot before/after LLM 调用，diff 即权威 artifact list，忽略 LLM 自报。Empty diff on `implement` → `SkillFailed` + 诊断信息（tokens 消耗、tool calls 数）。10 个新测试（7 unit + 3 e2e），187/187 ritual tests pass。

详见 `.gid/issues/ISS-038-implement-phase-no-output-postcondition.md`

### 关联
- ISS-029 (work-unit binding)
- ISS-016 (LSP dangling edges — adjacent surface)
- rustclaw ISS-016, ISS-025

---

## ISS-039 [bug] [P0] [closed]
...
## ISS-042 [hygiene] [P3] [closed]
**Clippy warnings cleanup — tracking issue (175 warnings)** *(2026-04-26)*

**发现日期**: 2026-04-26
**关闭日期**: 2026-04-26 — workspace clippy clean, 0 warnings under `-D warnings`
**发现者**: RustClaw (proactive scan after ISS-040/041)
**组件**: workspace-wide

### 问题
ISS-040/041 关掉 5 个 clippy errors 之后，`cargo clippy --all-targets --all-features` 仍有 175 warnings：~30 `too_many_arguments`、~25 `unnecessary_map_or`、~20 `collapsible_if`、~15 `manual_split_once`、~10 各种 autofix lint，加上长尾。其中 5 条 high-signal warnings 拆出独立 issue（ISS-043–046），其余在本 tracking issue 下批量清理。

### 修复
两阶段：Phase 1 autofix sweep（机械 lint）→ Phase 2 手动重构（`too_many_arguments` 用 ExtractCtx 模式 — 见 ISS-046）。最终 workspace clippy 在 `-D warnings` 下 0 warning。

详见 `.gid/issues/ISS-042-clippy-warnings-cleanup-tracking.md`

### 关联
- ISS-040, ISS-041 (errors 已修)
- ISS-043, ISS-044, ISS-045, ISS-046, ISS-047 (拆出的 high-signal)

---

## ISS-043 [bug] [P2] [closed]
**Python call-resolution confidence ladder collapsed `same_file` and `imported` tiers (both 0.8)** *(2026-04-26)*

**发现日期**: 2026-04-26
**关闭日期**: 2026-04-26
**发现者**: RustClaw (clippy `if_same_then_else`)
**组件**: `gid-core/src/code_graph/lang/python.rs` (`resolve_and_add_call_edge`, ~857-867)

### 问题
Python call resolution 的 confidence ladder 在 `same_file` 和 `imported` 两档都赋值 0.8 — 与同文件 line 932 的 graduated ladder（0.8/0.7/0.6）和 `rust_lang.rs:1855`（0.9/0.8/0.7）不一致。Git blame 显示是 `86f5b1c` refactor split 时的 copy-paste typo。

### 修复
`imported` 档 0.8 → 0.75（slot 在 same_file=0.8 与 same_pkg=0.7 之间，最小化对既有 edge weight 的扰动）。新增 `test_python_call_confidence_ladder_ordering` regression test。Doc comment 写明 ladder intent 与跨语言惯例。

详见 `.gid/issues/ISS-043-python-confidence-ladder-collapsed-tier.md`

### 关联
- ISS-042 (parent), ISS-012 (confidence-weighted edges)

---

## ISS-044 [perf] [P2] [closed]
**Python import-regex recompiled on every AST-walk iteration** *(2026-04-26)*

**发现日期**: 2026-04-26
**关闭日期**: 2026-04-26
**发现者**: RustClaw (clippy `regex_creation_in_loops`)
**组件**: `gid-core/src/code_graph/lang/{python.rs, typescript.rs}`, `code_graph/extract.rs`

### 问题
Python extractor 在 top-level AST walk 内每次循环都 `Regex::new(r"import\s+([\w.]+)")`，5k-file Python repo 估算浪费 5–25 秒纯 regex compile overhead。TypeScript 同模式。

### 修复
用 `once_cell` / `OnceLock` static 把 regex hoist 出循环，per-process 编译一次。

详见 `.gid/issues/ISS-044-python-regex-recompiled-per-loop-iteration.md`
...
## ISS-045 [hygiene] [P2] [closed]
**`infer/mod.rs` re-exports and uses its own deprecated constant `COLOCATION_PAIRWISE_LIMIT`** *(2026-04-26)*

**发现日期**: 2026-04-26
**关闭日期**: 2026-04-26
**发现者**: RustClaw (clippy `deprecated`)
**组件**: `gid-core/src/infer/{mod.rs, clustering.rs}`

### 问题
`COLOCATION_PAIRWISE_LIMIT` 已 deprecated（"co-location is now isolation-gated"），但 `infer/mod.rs:39` 仍在 re-export，自家 clippy 每次 build 都 warn。最坏的 deprecation：对外说 "别用"，对内还在导出。

### 修复
确认 migration 已完成（无内部 use，无 tracked 项目使用）→ 硬移除 constant 定义 + re-export。Clippy 干净，1122 lib tests pass。

详见 `.gid/issues/ISS-045-infer-uses-own-deprecated-constant.md`

### 关联
- ISS-042 (parent), ISS-013

---

## ISS-046 [refactor] [P3] [closed]
**Recursive extractors carry unused parameters that should be hoisted to caller-shared state** *(2026-04-26)*

**发现日期**: 2026-04-26
**关闭日期**: 2026-04-26
**发现者**: RustClaw (clippy `only_used_in_recursion`)
**组件**: `gid-core/src/code_graph/lang/rust_lang.rs` (`extract_rust_node`, `extract_calls_from_token_tree`)

### 问题
两个递归 AST walker 携带的参数（`impl_target_map`、`struct_field_types`）只被 forward 到下层递归，本层从不读 — dead weight。同时这两个函数也在 `too_many_arguments` lint 里（12/7 和 13/7 args）。Root cause 是共享状态被平铺成参数而非 group。

### 修复
Option A：引入 `ExtractCtx` struct 把共享 state 打包，5 个 phase 分批 land（A–E2），同时解掉 `only_used_in_recursion` 和 `too_many_arguments` 两条 lint。

详见 `.gid/issues/ISS-046-only-used-in-recursion-rust-extractors.md`

### 关联
- ISS-042 (parent), ISS-044 (类似 pattern)

---

## ISS-047 [bug] [P2] [closed]
**`gid tasks` summary still misleading — `manual` and `infer` source nodes uncounted (regression of ISS-034 fix)** *(2026-04-26)*

**发现日期**: 2026-04-26
**关闭日期**: 2026-04-26
**发现者**: RustClaw (after adding ISS-042..046 task nodes via `gid_add_task` to gid-rs's own graph)
**组件**: `gid-core/src/graph.rs` (`summary()`, `summary_text()`, Display impl)

### 问题
ISS-034 修了 project/code 二分 summary，但实际 `source` 字段有 4 类：`extract`（counted as code）、`infer`、`manual`、`project`/NULL（counted as project）。`infer` 和 `manual` 节点被 silently dropped — header 显示 "0 nodes" 而 body 列 33 个。Classification gap，不是 filter logic bug。

### 修复
Phase 1：summary buckets 显示所有 source category。Phase 2 (b)：`project_nodes()` 接受 `source="manual"`（manual 添加的 issue 是用户最关心的，不能消失）。7 个 regression tests，1122 lib tests green。

详见 `.gid/issues/ISS-047-tasks-summary-uncounted-source-categories.md`

### 关联
- ISS-034 (原 fix；本 issue 是其 generalization)

---

## ISS-048 [refactor] [P2] [closed]
**CodeNode Test Helper / Builder Pattern Refactor** *(2026-04-26)*

> Originally filed as "new ISS-001" on `iss-001-002-revive` branch — number collided with existing closed ISS-001 (`gid extract --output` flag). Renumbered 2026-04-26.

**发现日期**: 2026-04-02
**关闭日期**: 2026-04-26
**组件**: `gid-core/src/code_graph/types.rs` + 10 test sites

### 问题
`CodeNode` struct 字段不断扩张，每个 test 文件都要手写完整字段列表，新加字段时 10+ 处 test 同步修改成本高。

### 修复
新增 `CodeNode::test_default(id, kind)` helper，封装合理默认值。10 个 test site 用 functional record update (`..CodeNode::test_default(...)`) 重构。新增字段时只需更新 helper。

详见 `.gid/issues/ISS-048-test-helper-refactor.md`

---

## ISS-049 [improvement] [P1] [closed]
**Clustering Edge Weights Should Be Configurable** *(2026-04-26)*

> Originally filed as "new ISS-002" on `iss-001-002-revive` branch — number collided with existing closed ISS-002 (LSP client). Renumbered 2026-04-26.

**发现日期**: 2026-04-05
**关闭日期**: 2026-04-26
**组件**: `gid-core/src/infer/clustering.rs`, `gid-cli/src/main.rs`

### 问题
Infomap 聚类的 edge weights（按 relation 类型 calls/imports/co-citation/...）是硬编码的，不同项目最佳权重不同，无法 tune 就只能接受默认聚类质量。

### 修复
- 新增 `ClusterConfig` struct 携带 `edge_weights: HashMap<String, f64>` overrides
- `gid` CLI 新增 `--edge-weight RELATION=WEIGHT` flag（可重复，权重 0 = 忽略该 relation）
- `build_network` 用 `config.edge_weights` 查表，缺省回退到 `relation_weight()`
- 4 个新测试覆盖：override 生效、weight 0 跳过 edge、default 与 relation_weight 一致、两个 relation 同时 override

详见 `.gid/issues/ISS-049-configurable-edge-weights.md`

### 关联
- ISS-003, ISS-004, ISS-005, ISS-012 (clustering quality 系列；本 issue 提供 tuning 手段)
