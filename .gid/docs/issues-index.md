...
> **Note (2026-04-26):** ISS-001 和 ISS-002 的编号在历史上被复用过。
> 此 index 中的条目是**最初的**那批问题（gid extract --output flag, LSP client）。
> Branch `iss-001-002-revive` 解决的是**后来重新分配的** ISS-001（CodeNode test_default helper）和 ISS-002（configurable edge weights）。
> 详见独立文件 `.gid/issues/ISS-001-test-helper-refactor.md` 和 `.gid/issues/ISS-002-configurable-edge-weights.md`。
> 全部已 closed。
